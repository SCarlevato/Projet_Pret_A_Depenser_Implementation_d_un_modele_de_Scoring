...
def main() :
    ...
    def load_data_sample():
        sample_json = requests.get(URL_API + "/load_data/sample")
        sample = sample_json.json()
        return sample
    ...
    def load_data_target():
        target_json = requests.get(URL_API + "load/data/target")
        target = target_json.json()
        return target

    @st.cache
    def load_infos_gen_credit():
        infos_gen = requests.get(URL_API + "/load_infos_gen/credit")
    ...
    def load_income_population():
        df_income_json = request.get(URL_API + "/load_income_population")
